...
class Block (object):
	def __init__(self, transactions, time, index):
		...
		self.prev = '';
		self.nonse = 0;
		self.lotto = self.calculatelotto();
		self.hash = self.calculateHash();

	def calculatelotto(self):
		return "24 hr";
	...
	def calculateHash(self):
		...
		hashString = str(self.time) + hashTransactions + self.lotto + self.prev + str(self.nonse);
